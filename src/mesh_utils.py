...
class CBA_mesh():
    ...
    def load_cba(self, fname):
        ...
        for b in range(self.n_blocks):
            ...
            self.block[b].npts = self.block[b].npts_i * self.block[b].npts_j * self.block[b].npts_k
            ...
            self.block[b].n_cell = (self.block[b].npts_i - 1) * (self.block[b].npts_j - 1) * (self.block[b].npts_k - 1)
    ...
    def write_h5(self, fname='NONE'):
        ...
        h5mesh.create_dataset("faceNodes", data=self.faceNodes, shape=(self.n_face * 4, 1))
    ...
    def writeCBA(self, fname):
        ...
        for i in self.block:
            ...
            for j in range(self.block[i].npts):
                fout.write('{:.15f} \t {:.15f} \t {:.15f} \n'.format(self.block[i].X[j, 0], self.block[i].X[j, 1], self.block[i].X[j, 2]))
            for j in range(6):
                fout.write('{} \t {} \t {} \n'.format(int(self.block[i].connectivity[j]['type']), int(self.block[i].connectivity[j]['neighbour']), int(self.block[i].connectivity[j]['orientation'])))
        return
...
class CBA_block():
    ...
    def get_corners(self):
        ...
        for k in range(2):
            for j in range(2):
                for i in range(2):
                    corners[index, 0] = self.pts[i * (self.npts_i - 1), j * (self.npts_j - 1), k * (self.npts_k - 1), 0]
                    corners[index, 1] = self.pts[i * (self.npts_i - 1), j * (self.npts_j - 1), k * (self.npts_k - 1), 1]
                    corners[index, 2] = self.pts[i * (self.npts_i - 1), j * (self.npts_j - 1), k * (self.npts_k - 1), 2]
    ...
    def assign_primary_faces(self, cell_offset, face_offset):
        ...
        boundary_conditions = [0, self.npts_i - 2, 0, self.npts_j - 2, 0, self.npts_k - 2]                # Logical array to dictate boundary state
        ...
        if face_ID != self.n_face:
            print('Mismatch in face numbers: {} assigned, {} expected'.format(face_ID, self.n_face))
            print('Difference of {}'.format(self.n_face - face_ID))
        if cell_ID != self.n_cell:
            print('Mismatch in cell numbers: {} assigned, {} expected'.format(cell_ID, self.n_cell))
...
class zCFD_mesh:
    ...
    def write_boundary_tec(self, fname):
        ...
        surface_faceID = np.array(np.where(self.faceInfo[:, 0] != 0))[0, :]     # Find index of faces with non-zero face tag
        n_face = len(surface_faceID)                                        # Number of boundary faces
        surface_faceTag = np.zeros(n_face)                                  # array to store the tag of specific faces
        surface_faceNodes = np.zeros(n_face * 4)                              # array to store nodeID's of boundary faces
